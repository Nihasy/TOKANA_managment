...
export function computePrice({ zone, weightKg, isExpress }: PricingParams): number {
  ...
  if (weightKg <= 2) {
    basePrice = BASE_PRICES[zone].light
  } else if (weightKg <= 5) {
    basePrice = BASE_PRICES[zone].light + BASE_PRICES[zone].heavy
  } else {
    // For weights > 5kg, add additional charges per kg
    const extraKg = Math.ceil(weightKg - 5)
    basePrice = BASE_PRICES[zone].light + BASE_PRICES[zone].heavy + extraKg * 1000
  }

  // Add express surcharge if applicable
  if (isExpress) {
    basePrice += EXPRESS_SURCHARGE[zone]
  }
